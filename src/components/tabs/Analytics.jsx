...
export default function Analytics() {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/analytics')
            .then(res => res.json())
            .then(res => {
                setTrafficData(res.data.traffic_pattern);
                setVehicleTypes(res.data.distribution);
                setLoading(false);
            })
            .catch(err => {
    ...
    return (
        <div className="space-y-6">
            <div className="card border-l-4 border-l-cyan-500">
                ...
                <p className="text-slate-400 mt-2">Real-time traffic data and insights connected to Backend DB</p>
            </div>
